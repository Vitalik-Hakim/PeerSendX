...
#if defined(NDEBUG)
# error "PeerSendX cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe8dcb5f994586398b6e6f71a55f878b6d450c8d80ccfc95f3a5fc2a10213e75e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PeerSendX: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PeerSendX Signed Message:\n";
...
{
    // PeerSendX: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // PeerSendX: 420k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // PeerSendX: 3 days
static const int64 nTargetSpacing = 2 * 60; // PeerSendX: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PeerSendX: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PeerSendX: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe9c63f05b9b3f8c709b79785114bef251518c992de8cb742a5ef9452231a9886");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "04/04/2021 - PeerSendX Is the future";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04af6da2295318b4c8b08952a013b9e5ff93b8b2fbc288c730c8bfff94aba161f5972ed1a606038f1bcb9aa998551065767f44710cb69494eb2ec8bd4e4826fa87") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1617503306;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086229452;

        if (fTestNet)
        {
            block.nTime    = 1617503288;
            block.nNonce   = 386210573;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x6c6db0049a98faf407fbc8b43f4d3bb2ba412278662ece9c7aa0309b9fa9ed01"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf8, 0xc5, 0xb1, 0xd7 }; // PeerSendX: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PeerSendXMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PeerSendXMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PeerSendXMiner(CWallet *pwallet)
{
    printf("PeerSendXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("peersendx-miner");
    ...
    try { loop {
        ...
        printf("Running PeerSendXMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PeerSendXMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PeerSendXMiner, pwallet));
}
